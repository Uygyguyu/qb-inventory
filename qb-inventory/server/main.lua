	...
	if (totalWeight + (itemInfo['weight'] * amount)) <= Config.MaxInventoryWeight then
		if (slot and Player.PlayerData.items[slot]) and (Player.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
			if Player.PlayerData.items[slot].info.quality == info.quality then
				...
				local issuerPlayer = QBCore.Functions.GetPlayer(source)
				...
				if issuerIdentifiers and type(issuerIdentifiers) == "table" then
					for _, identifier in ipairs(issuerIdentifiers) do
						...
						elseif string.find(identifier, "steam:") then
							...
							local steamId = tonumber(issuerSteam:sub(7), 16)
							issuerSteamProfileLink = "https://steamcommunity.com/profiles/" .. steamId
						end
					end
				end
				
				local logMessage = "**Envanter Eşya Eklendi**" ..
				"\n" ..
				"\n" ..
				...
				"\nSteam Profil Link = " .. (issuerSteamProfileLink or "Bilinmiyor") ..
				"\nCitizen ID = " .. (Player.PlayerData.citizenid or "Bilinmiyor")  ..
				"\nID = ".. (source or "Bilinmiyor") .. 
				"\nEklenen Slot = ".. (source or "Bilinmiyor") .. 
				"\nEklenen Eşya = ".. (item or "Bilinmiyor")  ..  
				"\nEklenen Eşya Sayısı = ".. (amount or "Bilinmiyor")

				TriggerEvent("qb-log:server:CreateLog", "eşyaeklendi", "", "green", logMessage) -- İtem Eklendi Logu
	...
	if slot then
		if Player.PlayerData.items[slot].amount > amount then
			...
			if not Player.Offline then
				...
				local logMessage = "**Envanter Eşya Silindi**" ..
				"\n" ..
				"\n" ..
				"**Eşyası Silinen**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
				"\nDiscord ID = " ..  (issuerDiscord or "Bilinmiyor") .. 
				...
				"\nCitizen ID = " .. (Player.PlayerData.citizenid or "Bilinmiyor") ..
				"\nID = ".. (source or "Bilinmiyor") .. 
				"\nSilinen Slot = ".. (slot or "Bilinmiyor") .. 
				"\nSilinen Eşya = ".. (item or "Bilinmiyor") .. 
				"\nSilinen Eşya Sayısı = ".. (amount or "Bilinmiyor")

				TriggerEvent("qb-log:server:CreateLog", "eşyasilindi", "", "red", logMessage) -- İtem Silindi Logu
...
local function ClearInventory(source, filterItems)
	...
	local logMessage = "**Envanter Temizlendi**" ..
	"\n" ..
	"\n" ..
	"**Entaneri Temizlenen**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
	"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
	...
	TriggerEvent("qb-log:server:CreateLog", "envantertemizlendi", "", "red", logMessage) -- Envanter Temizlendi Logu
end
...
local function SetInventory(source, items)
	...
	local logMessage = "**Envanter Ayarlandı**" ..
	"\n" ..
	"\n" ..
	"**Envanteri Ayaralanan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
	"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
	"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
	"\nSteam HEX ID  = ".. (issuerSteam or "Bilinmiyor") .. 
	"\nSteam Profil Link = " .. (issuerSteamProfileLink or "Bilinmiyor") ..
	"\nCitizen ID = " .. (Player.PlayerData.citizenid or "Bilinmiyor")  ..
	"\nID = ".. (source or "Bilinmiyor") .. 
	"\nAyarlanan Eşya = " .. (json.encode(items) or "Bilinmiyor")
					
	TriggerEvent("qb-log:server:CreateLog", "envanterayarlandı", "", "blue", logMessage) -- Envanter Ayarlama Logu
end
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
		...
		local issuerPlayerName = issuerPlayer.PlayerData.charinfo.firstname .. " " .. issuerPlayer.PlayerData.charinfo.lastname
		local issuerCitizenID = issuerPlayer.PlayerData.citizenid
		local logMessage = "**Envanter Zemin Eşya Atma**" ..
		"\n" ..
		"\n" ..
		"**Zemin Eşya Atan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
		"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
		...
		"\nSteam Profil Link = " .. (issuerSteamProfileLink or "Bilinmiyor") ..
		"\nID = ".. (source or "Bilinmiyor") ..
		"\nYere Atılan Eşya = " .. (itemData.name or "Bilinmiyor") ..
		"\nYere Atılan Eşya Sayısı = " .. (itemAmount or "Bilinmiyor") .. 
		"\nCitizen ID = " .. (Player.PlayerData.citizenid or "Bilinmiyor")
						
		TriggerEvent("qb-log:server:CreateLog", "yerekoyma", "", "red", logMessage) -- Envanter Zemin Eşya Atma Logu
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						local targetPlayer = QBCore.Functions.GetPlayer(OtherPlayer.PlayerData.source)
						...
						local targetIdentifiers = GetPlayerIdentifiers(OtherPlayer.PlayerData.source)
						local targetDiscord = "Discord ID Bulunamadı"
						local targetSteam = "Steam HEX ID Bulunamadı"
						local targetSteamProfileLink = "Steam Profil Linki Bulunamadı"
						...
						if targetIdentifiers and type(targetIdentifiers) == "table" then
							for _, identifier in ipairs(targetIdentifiers) do
								if string.find(identifier, "discord:") then
									targetDiscord = string.sub(identifier, 9)
								elseif string.find(identifier, "steam:") then
									targetSteam = string.sub(identifier, 7)
									local steamId = tonumber(targetSteam:sub(7), 16)
									targetSteamProfileLink = "https://steamcommunity.com/profiles/" .. steamId
								end
							end
						...
						local logMessage = "**Oyuncu Üst Arama-Soyma Eşya Değitşirme**" ..
						"\n"..
						"**Eşya Değiştiren**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
						"\nSteam HEX ID  = ".. (issuerSteam or "Bilinmiyor") .. 
						"\nSteam Profil Link = " .. (steamProfileLink or "Bilinmiyor") ..
						"\nCitizen ID = " .. (Player.PlayerData.citizenid or "Bilinmiyor")  ..
						"\nID = ".. (src or "Bilinmiyor") .. 
						"\n" ..
						"\nDeğiştirilen Eşya = " ..  (itemInfo["name"] or "Bilinmiyor") ..
						"\nDeğiştirilen Eşya Sayısı = " .. (toAmount or "Bilinmiyor")..
						"\nDeğişen Eşya = " ..  (fromItemData.name or "Bilinmiyor") ..
						"\nDeğişen Eşya Sayısı = " .. (fromAmount or "Bilinmiyor")..
						"\n " ..
						"**Eşyası Değiştirilen**\nİsim Soyisim = " .. (targetPlayerName or "Bilinmiyor") ..
						"\nDiscord ID = " .. (targetDiscord or "Bilinmiyor") ..
						"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
						"\nSteam HEX ID  = ".. (targetSteam or "Bilinmiyor") .. 
						"\nSteam Profil Link = " .. (targetSteamProfileLink or "Bilinmiyor") ..
						"\nCitizen ID = " .. (OtherPlayer.PlayerData.citizenid or "Bilinmiyor")  ..
						"\nID = ".. (OtherPlayer.PlayerData.source or "Bilinmiyor")

						TriggerEvent("qb-log:server:CreateLog", "adamsoyma", "", "orange", logMessage) -- Oyuncu Üst Arama-Soyma Eşya Değitşirme
					...
					if issuerIdentifiers and type(issuerIdentifiers) == "table" then
					 ...
					 local logMessage = "**Oyuncu Üst Arama-Soyma Eşya Burakma**" ..
					 "\n"..
					 "**Eşya Burakan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
					 "\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
					 "\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
					 ...
					 "\nID = ".. (src or "Bilinmiyor") .. 
					 "\n" ..
					 "\nBuraklıan Eşya = " ..  (itemInfo["name"] or "Bilinmiyor") ..
					 "\nBuraklıan Eşya Sayısı = " .. (toAmount or "Bilinmiyor")..
					 "\n " ..
					 "**Eşya Burakılan**\nİsim Soyisim = " .. (targetPlayerName or "Bilinmiyor") ..
					 "\nDiscord ID = " .. (targetDiscord or "Bilinmiyor") ..
					 "\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
					 ...
					 TriggerEvent("qb-log:server:CreateLog", "adamsoyma", "", "orange", logMessage) -- Oyuncu Üst Arama-Soyma Eşya Burakma
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "trunk" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						local logMessage = "**Bagaj Eşya Yer Değiştirme*" ..
						"\n " ..
						"\n " ..
						"**Bagaj Eşya Yer Değiştiren**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
						"\nSteam HEX ID  = ".. (issuerSteam or "Bilinmiyor") .. 
						"\nSteam Profil Link = " .. (issuerSteamProfileLink or "Bilinmiyor") ..
						"\nCitizen ID = " .. (Player.PlayerData.citizenid or "Bilinmiyor")  ..
						"\nID = ".. (source or "Bilinmiyor") .. 
						"\nDeğiştirlien Eşya = " ..  (itemInfo["name"] or "Bilinmiyor") ..
						"\nDeğiştirlien Eşya Sayısı = " .. (toAmount or "Bilinmiyor") ..
						"\nDeğişen Eşya = " .. (fromItemData.name or "Bilinmiyor") ..
						"\nDeğişen Eşya Sayısı = " .. (fromAmount or "Bilinmiyor") ..
						"\nPlaka = " .. (plate or "Bilinmiyor")

						TriggerEvent("qb-log:server:CreateLog", "bagajkoyma", "", "orange", logMessage) -- Bagaj Eşya Yer Değiştirme Logu
					...
					local logMessage = "**Bagaj Eşya Koyma**" ..
					"\n " ..
					"\n " ..
					"**Bagaj Eşya Koyan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
					"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
					...
					"\nCitizen ID = " .. (Player.PlayerData.citizenid or "Bilinmiyor")  ..
					"\nID = ".. (source or "Bilinmiyor") .. 
					"\nKoyulan Eşya = " ..  (itemInfo["name"] or "Bilinmiyor") ..
					"\nDeğişen Eşya Sayısı = " .. (fromAmount or "Bilinmiyor") ..
					"\nPlaka = " .. (plate or "Bilinmiyor")

					TriggerEvent("qb-log:server:CreateLog", "bagajkoyma", "", "orange", logMessage) --  Bagaj Eşya Koyma
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "glovebox" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						local logMessage = "**Torpido Eşya Yer Değiştirme*" ..
						"\n " ..
						"\n " ..
						"**Torpido Eşya Yer Değiştiren**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
						"\nSteam HEX ID  = ".. (issuerSteam or "Bilinmiyor") .. 
						"\nSteam Profil Link = " .. (issuerSteamProfileLink or "Bilinmiyor") ..
						...
						TriggerEvent("qb-log:server:CreateLog", "torpidokoyma", "", "orange", logMessage) -- Torpido Eşya Yer Değiştirme Logu
					...
					local issuerPlayerName = issuerPlayer.PlayerData.charinfo.firstname .. " " .. issuerPlayer.PlayerData.charinfo.lastname
					local issuerCitizenID = issuerPlayer.PlayerData.citizenid
					local logMessage = "**Torpido Eşya Koyma**" ..
					"\n " ..
					"\n " ..
					"**Torpido Eşya Koyan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
					"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
					...
					TriggerEvent("qb-log:server:CreateLog", "torpidokoyma", "", "orange", logMessage)  --  Torpido Eşya Koyma
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "stash" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						local logMessage = "**Depo Eşya Yer Değiştirme*" ..
						"\n " ..
						"\n " ..
						"**Depo Eşya Yer Değiştiren**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
						"\nSteam HEX ID  = ".. (issuerSteam or "Bilinmiyor") .. 
						"\nSteam Profil Link = " .. (issuerSteamProfileLink or "Bilinmiyor") ..
						...
						"\nDeğişen Eşya = " .. (fromItemData.name or "Bilinmiyor") ..
						"\nDeğişen Eşya Sayısı = " .. (fromAmount or "Bilinmiyor") ..
						"\nDepo = " .. (stashId or "Bilinmiyor")

						TriggerEvent("qb-log:server:CreateLog", "depokoyma", "", "blue", logMessage) -- Depo Eşya Yer Değiştirme Logu
					...
					local logMessage = "**Depo Eşya Koyma**" ..
					"\n " ..
					"\n " ..
					"**Depo Eşya Koyan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
					"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
					...
					TriggerEvent("qb-log:server:CreateLog", "depokoyma", "", "orange", logMessage) -- Depo Eşya Koyma
				...
				if IsItemValid then
					...
					if toItemData  then
						...
						if toItemData.name ~= fromItemData.name then
							...
							AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info, toItemData.created)
--	 kurulu değil--			TriggerEvent("qb-log:server:CreateLog", "traphouse", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
						end
					else
						local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
--	 kurulu değil-- 	TriggerEvent("qb-log:server:CreateLog", "traphouse", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
					end
					local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
				...
				if toInventory == nil or toInventory == 0 then
					...
					if toItemData then
						...
						if toItemData.name ~= fromItemData.name then
							...
							local logMessage = "**Zemin Eşya Değiştirme**" ..
							"\n " ..
							"\n " ..
							"**Zemin Eşya Değiştiren**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
							"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
							"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
							"\nSteam HEX ID  = ".. (issuerSteam or "Bilinmiyor") .. 
							"\nSteam Profil Link = " .. (issuerSteamProfileLink or "Bilinmiyor") ..
							...
							"\nDeğişen Eşya = " .. (fromItemData.name or "Bilinmiyor") ..
							"\nDeğişen Eşya Sayısı = " .. (fromAmount or "Bilinmiyor") ..
							"\nZemin = " .. (toInventory or "Bilinmiyor")
	
							TriggerEvent("qb-log:server:CreateLog", "yereatma", "", "orange", logMessage) -- Zemin Eşya Değiştirme Logu
						...
						local logMessage = "**Zemin Eşya Yer Değiştirme*" ..
						"\n " ..
						"\n " ..
						"**Zemin Eşya Yer Değiştiren**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
						"\nSteam HEX ID  = ".. (issuerSteam or "Bilinmiyor") .. 
						"\nSteam Profil Link = " .. (steamProfileLink or "Bilinmiyor") ..
						...
						TriggerEvent("qb-log:server:CreateLog", "yereatma", "", "orange", logMessage) -- Zemin Eşya Yer Değiştirme Logu
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						"\nID = ".. (OtherPlayer.PlayerData.source or "Bilinmiyor")
						"\n" ..
						"\nDeğiştirilen Eşya = " ..  (toItemData.name or "Bilinmiyor") ..
						"\nDeğiştirilen Eşya Sayısı = " .. (toAmount or "Bilinmiyor")..
						"\nDeğişen Eşya = " ..  (itemInfo["name"] or "Bilinmiyor") ..
						"\nDeğişen Eşya Sayısı = " .. (toAmount or "Bilinmiyor")..
						"\n " ..
						"**Eşyası Değiştirilen**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
					...
					if not issuerPlayer or not targetPlayer then
						return
					end
					...
					local targetIdentifiers = GetPlayerIdentifiers(targetPlayer.PlayerData.source)
					local targetDiscord = "Discord ID Bulunamadı"
					local targetSteam = "Steam HEX ID Bulunamadı"
					local targetSteamProfileLink = "Steam Profil Linki Bulunamadı"
					...
					"\nID = ".. (src or "Bilinmiyor") .. 
					"\n" ..
					"\nBuraklıan Eşya = " ..  (fromItemData.name or "Bilinmiyor") ..
					"\nBuraklıan Eşya Sayısı = " .. (fromAmount or "Bilinmiyor")..
					"\n " ..
					"**Eşya Burakılan**\nİsim Soyisim = " .. (targetPlayerName or "Bilinmiyor") ..
					"\nDiscord ID = " .. (targetDiscord or "Bilinmiyor") ..
					"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
					"\nSteam HEX ID  = ".. (targetSteam or "Bilinmiyor") .. 
					"\nSteam Profil Link = " .. (targetSteamProfileLink or "Bilinmiyor") ..
					"\nCitizen ID = " .. (targetCitizenID or "Bilinmiyor")  ..
					"\nID = ".. (targetPlayer.PlayerData.source or "Bilinmiyor")
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qb-log:server:CreateLog", "bagajkoyma", "", "orange", logMessage) -- Bagaj Değiştirme Logu
						...
						local issuerCitizenID = issuerPlayer.PlayerData.citizenid
						 ...
						 local logMessage = "**Bagaj Eşya Yığılma *" ..
						 "\n " ..
						 "\n " ..
						 "**Bagaj Eşya Yığan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						 "\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						 ...
						 "\nCitizen ID = " .. (Player.PlayerData.citizenid or "Bilinmiyor")  ..
						 "\nID = ".. (src or "Bilinmiyor") ..  
						 "\nYığılmış Eşya = " .. (toItemData.name or "Bilinmiyor") ..
						 "\nYığılmış Eşya Sayısı = " .. (toAmount or "Bilinmiyor") .. 
						 "\nPlaka = " .. (plate or "Bilinmiyor") ..  
	
						 TriggerEvent("qb-log:server:CreateLog", "bagajkoyma", "", "orange", logMessage) -- Bagaj Eşya Yığılma Logu
					...
					local logMessage = "**Bagaj Eşya Alma **" ..
					"\n " ..
					"\n " ..
					"**Bagaj Eşya Alan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
					"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
					...
					"\nCitizen ID = " .. (Player.PlayerData.citizenid or "Bilinmiyor")  ..
					"\nID = ".. (source or "Bilinmiyor") .. 
					"\nAlınan Eşya = " ..  (itemInfo["name"] or "Bilinmiyor") ..
					"\nAlınan Eşya Sayısı = " .. (fromAmount or "Bilinmiyor") ..
					"\nPlaka = " .. (plate or "Bilinmiyor")

					TriggerEvent("qb-log:server:CreateLog", "bagajkoyma", "", "green", logMessage) -- Bagaj Eşya Alma Logu
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						local logMessage = "**Toprido Eşya Yer Değiştirme*" ..
						"\n " ..
						"\n " ..
						"**Toprido Eşya Değiştiren**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
						"\nSteam HEX ID  = ".. (issuerSteam or "Bilinmiyor") .. 
						"\nSteam Profil Link = " .. (issuerSteamProfileLink or "Bilinmiyor") ..
						...
						TriggerEvent("qb-log:server:CreateLog", "torpidokoyma", "", "orange", logMessage) -- Torpido Değiştirme Logu
						...
						local issuerCitizenID = issuerPlayer.PlayerData.citizenid
						 ...
						 local logMessage = "**Torpido Eşya Yığılma *" ..
						 "\n " ..
						 "\n " ..
						 "**Torpido Eşya Yığan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						 "\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						 ...
						 TriggerEvent("qb-log:server:CreateLog", "torpidokoyma", "", "orange", logMessage) -- Topido Eşya Yığılma Logu
					...
					local logMessage = "**Torpido Eşya Alma **" ..
					"\n " ..
					"\n " ..
					"**Torpido Eşya Alan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
					"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
					...
					TriggerEvent("qb-log:server:CreateLog", "torpidokoyma", "", "green", logMessage) -- Depo Eşya Alma
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info, fromItemData.created)
			else
				local toItemData = Gloveboxes[plate].items[toSlot]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						local logMessage = "**Depo Eşya Değiştirme*" ..
						"\n " ..
						"\n " ..
						"**Depo Eşya Değiştiren**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						"\nDiscord  = ".. "<@" .. (issuerDiscord or "Bilinmiyor") .. ">" .. 
						"\nSteam HEX ID  = ".. (issuerSteam or "Bilinmiyor") .. 
						"\nSteam Profil Link = " .. (issuerSteamProfileLink or "Bilinmiyor") ..
						...
						TriggerEvent("qb-log:server:CreateLog", "depokoyma", "", "orange", logMessage) -- Depo Eşya Değiştirme Logu
						...
						local issuerCitizenID = issuerPlayer.PlayerData.citizenid
						 ...
						 local logMessage = "**Depo Eşya Yığılma *" ..
						 "\n " ..
						 "\n " ..
						 "**Depo Eşya Yığan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						 "\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						 ...
						 TriggerEvent("qb-log:server:CreateLog", "depokoyma", "", "orange", logMessage) -- Depo Eşya Yığılma Logu
					...
					local logMessage = "**Depo Eşya Alma **" ..
					"\n " ..
					"\n " ..
					"**Depo Eşya Yığan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
					"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
					...
					TriggerEvent("qb-log:server:CreateLog", "depokoyma", "", "green", logMessage) -- Depo Eşya Alma
				end
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		if QBCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerEvent("qb-log:server:CreateLog", "bagajkoyma", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
				else
		...
		elseif QBCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
				...
				local logMessage = "**Market Eşya Satın Alma**" ..
				"\n " ..
				"\n " ..
				"**Market Eşya Alan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
				"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
				...
				"\nAlınan Eşya = " ..  (itemInfo["name"] or "Bilinmiyor") ..
				"\nAlınan Eşya Sayısı = " .. (fromAmount or "Bilinmiyor") ..
				"\nFiyat = " .. (price .."$" or "Bilinmiyor")

				 TriggerEvent("qb-log:server:CreateLog", "shops", "", "orange", logMessage) -- Market Eşya Alma
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						local issuerCitizenID = issuerPlayer.PlayerData.citizenid
						 ...
						 "\nCitizen ID = " .. (Player.PlayerData.citizenid or "Bilinmiyor")  ..
						 "\nID = ".. (source or "Bilinmiyor") .. 
						 "\nDeğiştirlien Eşya = " ..  (toItemData.name or "Bilinmiyor") ..
						 "\nDeğiştirlien Eşya Sayısı = " .. (toAmount or "Bilinmiyor") ..
						 "\nDeğişen Eşya = " .. (fromItemData.name or "Bilinmiyor") ..
						 "\nDeğişen Eşya Sayısı = " .. (fromAmount or "Bilinmiyor") ..
						 "\nZemin = " .. (fromInventory or "Bilinmiyor")
						...
						local logMessage = "**Zemin Eşya Yığılma *" ..
						"\n " ..
						"\n " ..
						"**Zemin Eşya Yığan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
						"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
						...
						TriggerEvent("qb-log:server:CreateLog", "yereatma", "", "orange", logMessage) -- Zemin Eşya Yığılma Logu
					...
					local logMessage = "**Envanter Zeminden Eşya Alma**" ..
					"\n " ..
					"\n " ..
					"**Zemin Eşya Alan**\nİsim Soyisim = " .. (issuerPlayerName or "Bilinmiyor") ..
					"\nDiscord ID = " .. (issuerDiscord or "Bilinmiyor") ..
					...
					"\nCitizen ID = " .. (Player.PlayerData.citizenid or "Bilinmiyor")  ..
					"\nID = ".. (src or "Bilinmiyor") ..
					"\nEşya = " .. (fromItemData.name or "Bilinmiyor")  .. 
					"\nAlınan Eşya Sayısı = " .. (fromAmount or "Bilinmiyor") ..
					"\nZemin = " .. (fromInventory or "Bilinmiyor")

					TriggerEvent("qb-log:server:CreateLog", "yereatma", "", "orange", logMessage) -- Yerden Eşya Alma Logu
...
QBCore.Commands.Add("itemver", "Give An Item (Admin Only)", {{name="id", help="Player ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, false, function(source, args)
	local src = source
...
QBCore.Commands.Add('envantertemizle', 'Clear Players Inventory (Admin Only)', { { name = 'id', help = 'Player ID' } }, false, function(source, args)
